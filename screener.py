...
def calc_smart_carry_yield(funding_hist, highs, lows, closes):
    """
    Smart Cash-and-Carry Metric:
    Calculates Funding Yield / Historical Volatility (ATR).
    High positive funding + low volatility = low turnover cost for carry.
    Returns: float
    """
    if not funding_hist or len(funding_hist) < 1:
        return 0.0

    current_fund = funding_hist[-1]
    ...
    price = closes[-1]
    volatility_pct = (atr_val / price) * 100 if price > 0 else 1.0

    vol_floor = max(volatility_pct, 0.1)

    carry_ratio = current_fund / vol_floor
    return round(carry_ratio, 3)
...
def score_symbol(symbol, ticker, oi_hist,
    ...
    # --- MAKER EDGE: Toxic vs Forced Flow Detection & Smart Carry ---
    flow_type = "neutral"
    forced_flow_ratio = 0.0
    maker_flow = ""
    smart_carry = 0.0

    # Calculate Smart Cash-and-Carry
    if funding_hist and len(funding_hist) > 0:
        smart_carry = calc_smart_carry_yield(funding_hist, hi1h, lo1h, cl1h)

    # Liq DB check vs Volume
    if liq_stats and symbol in liq_stats:
        liq_data = liq_stats[symbol]
        total_liq = float(liq_data.get('short_usd', 0)) + float(liq_data.get('long_usd', 0))

        med_vol = 0
        if len(vol1h) > 2:
            med_vol = sorted(vol1h[:-1])[len(vol1h[:-1])//2]

        est_vol_usd = med_vol * price

        if est_vol_usd > 0 and total_liq > 0:
            forced_flow_ratio = total_liq / est_vol_usd

            if vol_ratio > 1.5 and forced_flow_ratio < 0.01:
                flow_type = "toxic"
                maker_flow = "TOXIC"
            elif forced_flow_ratio > 0.15:
                flow_type = "forced"
                maker_flow = f"FORCED({forced_flow_ratio:.2f})"

    return {
        "symbol":    symbol,
        "maker_flow": maker_flow,
        "smart_carry": smart_carry,
        "price":     price,
        "fund_%":    funding,
        "basis_%":   basis_pct,
        "oi24h_%":   oi_change,
