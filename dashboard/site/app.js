(() => {
  ...
  function episodeCard(episode) {
    ...
    let makerTags = '';
    if (episode.maker_flow) {
        makerTags += `<span class="badge ${episode.maker_flow.includes('TOXIC') ? 'negative' : 'positive'}">${esc(episode.maker_flow)}</span> `;
    }
    if (episode.smart_carry) {
        makerTags += `<span class="badge neutral" title="Smart Carry Ratio">⚖️ ${Number(episode.smart_carry).toFixed(2)}</span>`;
    }

    return `<a class="card episode" href="#/episode/${encodeURIComponent(episode.id)}">
      <div class="row"><div><p class="kicker">${esc(episode.eventLabel)}</p><h3>${esc(episode.symbol || 'Неидентифицированный инструмент')} ${makerTags}</h3></div>${badge(episode.status)}</div>
  ...
  function nowObservationCard(observation) {
    ...
    // Add Maker Edge facts
    if (observation.maker_flow) {
        const flowBadge = observation.maker_flow.includes('TOXIC') ? 'negative' : 'positive';
        actualFacts.push(['Maker Flow', `<span class="badge ${flowBadge}">${esc(observation.maker_flow)}</span>`]);
    }
    if (observation.smart_carry) {
        actualFacts.push(['Smart Carry', `${Number(observation.smart_carry).toFixed(2)}`]);
    }
  ...
  function positioningWorkbench(model) {
    ...
    return `<section class="positioning-workbench" aria-label="Positioning crypto COT research">
      ...
      ${dataState}
      <p class="positioning-note">После накопления фактов слой будет держать максимум 3–5 independent исследовательских кейсов с жизненным циклом WATCH → ARMED → INVALIDATED → COOLDOWN.</p>
